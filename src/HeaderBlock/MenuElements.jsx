...
const MenuElements = ({outer_class, inner_class, massiv, style_elements, bolder_check}) => {
    ...
    return (
        <div className={outer_class}>
            ...
            {
                massiv.map(elementMenu => <ElementMenu elementMenu={elementMenu} key={elementMenu.id} style={((bolder_check === elementMenu.id)&&(active===false)) ? "bolder_elem" : style_elements}/>)
            }
            </motion.div>
        </div>
    );
